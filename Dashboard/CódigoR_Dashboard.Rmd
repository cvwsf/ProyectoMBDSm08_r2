...
### Tabla resumen de motivaciones por país, sexo y categoría de edad



Column {data-width=33%}
-------------------------------------------------
## Tabla de las medias de motivaciones por país
```{r}

# Media de las motivaciones por país
summary_country <- df_filtered %>%
  group_by(cntry) %>%
  summarise(mean_motprosoc = round(mean(motprosoc, na.rm = TRUE), 2),
            mean_motextr = round(mean(motextr, na.rm = TRUE), 2),
            mean_motintr = round(mean(motintr, na.rm = TRUE), 2))

# Verificar la estructura de summary_country
print(summary_country)

# Mostrar la tabla resumen usando DT
datatable(summary_country, 
          caption = 'Media de las motivaciones por país', 
          rownames = FALSE, 
          filter = 'top', 
          options = list(pageLength = 15, autoWidth = TRUE, paging = TRUE, searching = TRUE))
...
Column {data-width=33%}
-------------------------------------------------
## Tabla de las medias de motivaciones por sexo

```{r}
# Media de las motivaciones por sexo
summary_sex <- df_filtered %>%
  group_by(gndr) %>%
  summarise(mean_motprosoc = round(mean(motprosoc, na.rm = TRUE), 2),
            mean_motextr = round(mean(motextr, na.rm = TRUE), 2),
            mean_motintr = round(mean(motintr, na.rm = TRUE), 2))

# Verificar la estructura de summary_sex
print(summary_sex)

# Mostrar la tabla resumen usando DT
datatable(summary_sex, 
          caption = 'Media de las motivaciones por sexo', 
          rownames = FALSE, 
          filter = 'top', 
          options = list(pageLength = 15, autoWidth = TRUE, paging = TRUE, searching = TRUE))
...
Column {data-width=33%}
-------------------------------------------------
## Tabla de las medias de motivaciones por categoría de edad

```{r}
# Media de las motivaciones por categoría de edad
summary_age <- df_filtered %>%
  group_by(age_category) %>%
  summarise(mean_motprosoc = round(mean(motprosoc, na.rm = TRUE), 2),
            mean_motextr = round(mean(motextr, na.rm = TRUE), 2),
            mean_motintr = round(mean(motintr, na.rm = TRUE), 2))

# Verificar la estructura de summary_age
print(summary_age)

# Mostrar la tabla resumen usando DT
datatable(summary_age, 
          caption = 'Media de las motivaciones por categoría de edad', 
          rownames = FALSE, 
          filter = 'top', 
          options = list(pageLength = 15, autoWidth = TRUE, paging = TRUE, searching = TRUE))
